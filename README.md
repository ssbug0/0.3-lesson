...
Механизм отбора валидаторов в сети функционирует на базе алгоритма, который учитывает несколько ключевых параметров. Главный из них- объём заблокированных ETH (размер стейка): чем он больше, тем выше вероятность, что конкретный валидатор будет выбран для формирования нового блока или подтверждения уже созданных блоков другими участниками сети.

Помимо величины стейка, система принимает во внимание фактор случайности:
* для обеспечения непредвзятости процесса;
* срок нахождения средств в стейкинге (продолжительность времени блокировки ETH) - старшие стейки получают определённые преимущества.
...
| Скорость транзакций | Механизм консенсуса PoS и DPoS обеспечивают наиболее высокие скорости транзакций (TPS) в отличии от PoW по причине отсутствия необходимости выполнения оборудованием ресурсоемких операций.Блокчейны построенные на PoW консенсусе как правило ограничены 100-1000 TPS, когда PoS и DPoS могут предложить от 10000 до 1 млн TPS в зависимости от масштабирования сети.|
...
Произведен общий анализ и сопоставления популярных механизмов консенсуса по скорости обработки транзакций, безопасности и требованиям к оборудованию. Изучены основные типы уязвимостей блокчейнов(long-range, Sybil, nothing at stake, 51% атаки).
